...
export default function Profile() {
  ...
  const handleSaveBio = async () => {
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : "تأكد من إعدادات الجدول والاتصال";
      ...
      <div className="grid lg:grid-cols-[7fr_3fr] gap-8">
            ...
            <section className="bg-card border border-border rounded-xl p-6 relative overflow-hidden">
              ...
              <div className="flex items-center gap-2.5 mb-4">
                ...
                <h2 className="font-bold text-base text-foreground">
                  بيئة اختبار بوابة الدفع والدراسة (Dev Sandbox)
                </h2>
              </div>

              <p className="text-xs text-muted-foreground mb-5 leading-relaxed">
                هذه اللوحة تظهر لك فقط لأنك في وضع التطوير المحلي (`localhost`). تتيح لك التنقل
                السريع واختبار تفعيل الباقات مجاناً.
              </p>
        ...
        <aside className="space-y-5 lg:sticky lg:top-24 self-start">
          ...
          <div className="bg-card border border-border rounded-xl p-5 text-center">
            ...
            <span
              className={`inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full border transition-all duration-300 ${activeTierProps.colorClass}`}
            >
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            ...
            {bioSuccess && (
              <p className="text-xs text-emerald-600 mt-1 leading-tight">
                ✓ تم حفظ النبذة بنجاح في قاعدة البيانات!
              </p>
            )}
