...
// Translations
const txt = {
  lockedTitle: t4("ميزة حصرية للمشتركين", "Fonctionnalité exclusive Premium", "Premium Exclusive Tool", "Función exclusiva Premium"),
  lockedDesc: t4(
  ...
  unlockBtn: t4("ترقية الحساب الآن", "Débloquer la version Premium", "Unlock Premium Instantly", "Desbloquear Premium ahora"),
  sponsoredTag: t4("إعلان ممول", "Annonce sponsorisée", "Sponsored Advertisement", "Anuncio patrocinado"),
  adTitle: t4("إعلان غوغل التفاعلي", "Espace publicitaire interactif Google", "Google Responsive Ad Slot", "Espacio publicitario interactivo de Google"),
  adSubtitle: t4("تختفي هذه الإعلانات تلقائياً عند الاشتراك.", "Ces annonces disparaissent automatiquement avec un abonnement.", "Upgrade to Premium to remove all display banners.", "Actualice a Premium para eliminar todos los anuncios."),
  adSimulation: t4("محاكاة النقرات على الإعلان", "Simulation de clic publicitaire", "Ad click simulation", "Simulación de clic publicitario"),
  adNotConfigured: t4("لم يتم تكوين AdSense. قم بضبط VITE_GOOGLE_ADSENSE_CLIENT_ID في بيئتك.", "AdSense non configuré. Définissez VITE_GOOGLE_ADSENSE_CLIENT_ID dans votre environnement.", "AdSense client not configured. Set VITE_GOOGLE_ADSENSE_CLIENT_ID in your environment.", "AdSense no configurado. Establezca VITE_GOOGLE_ADSENSE_CLIENT_ID en su entorno."),
};
...
export default function MonetizationWrapper({
  ...
  useEffect(() => {
    if (!AD_CLIENT_ID || !shouldShowAds || adPushedRef.current || typeof window === "undefined") return;
    ...
    if (typeof window.requestIdleCallback === "function") {
      const idleId = window.requestIdleCallback(loadAdsenseScript, { timeout: 2000 });
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center p-8 min-h-[150px]" aria-label="Loading content status">
  ...
  if (lockFeature && !isPremium) {
    return (
      <div className="relative border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden my-6 bg-white dark:bg-slate-900 shadow-xs transition-colors" dir={dir}>
        ...
        <div className="absolute inset-0 bg-white/60 dark:bg-slate-950/70 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center z-10">
          ...
          >
            ...
            <ArrowRight size={13} className="shrink-0 rtl:rotate-180" aria-hidden="true" />
          </Link>
        </div>
      </div>
    );
      ...
      <div className="space-y-4" dir={dir}>
        ...
        <div className="w-full border border-dashed border-slate-300 dark:border-slate-700 rounded-xl p-4 bg-slate-50/70 dark:bg-slate-900/40 text-center my-6 relative overflow-hidden transition-colors">
          ...
          <div className="flex flex-col items-center justify-center min-h-[100px] text-slate-500 dark:text-slate-400">
            <span className="text-xs font-semibold text-slate-700 dark:text-slate-300" style={{ fontFamily: sansFont(lang) }}>
              {txt.adTitle[lang]}
            </span>
            <span className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5" style={{ fontFamily: sansFont(lang) }}>
              {txt.adSubtitle[lang]}
            </span>
