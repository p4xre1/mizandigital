...
// page view is attributed and stored for admin analytics.
...
function classifyReferrer(ref: string): { source: string; medium: string } {
  ...
  if (host && typeof window !== "undefined" && host !== window.location.hostname) return { source: host, medium: "referral" };
