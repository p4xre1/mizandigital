    ...
    <>
      ...
      <main className="min-h-screen bg-white dark:bg-black" dir="rtl">
        <div className="container mx-auto max-w-[1280px] px-4 py-6">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-6 border-b-2 border-[#1e293b] dark:border-white pb-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="size-1 h-5 bg-red-600" />
                <h1 className="text-[24px] font-black tracking-[-0.02em]">{pageTitle}</h1>
                <span className="bg-black dark:bg-white dark:text-black text-white text-[10px] px-2 py-0.5 rounded font-bold">{filteredItems.length} خبر</span>
              </div>
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <input type="text" placeholder="ابحث..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="h-9 w-64 rounded-full border border-border dark:border-white/10 bg-white dark:bg-[#1a1a1a] pr-9 pl-3 text-[12px] outline-none focus:border-[#1e293b]/20 dark:focus:border-white/20" />
              </div>
          ...
          <div className="bg-red-600 dark:bg-red-700 text-white rounded mb-6">
            <div className="px-4 h-9 flex items-center gap-3 text-[12px]">
              <span className="bg-black dark:bg-white dark:text-black px-3 py-1 rounded text-[10px] font-black shrink-0">BREAKING NEWS</span>
          ...
          {loading ? (
            <div className="grid grid-cols-12 gap-6">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="col-span-4 h-64 bg-white border border-border rounded animate-pulse" />
              ))}
            </div>
            ...
            <>
              <div className="grid grid-cols-12 gap-6">
                <div className="col-span-12 lg:col-span-3">
                  ...
                  <div className="space-y-3">
                    {editorsPicks.map((item) => (
                      <Link key={item.id} to={`/news/${item.slug || item.id}`} className="group flex gap-3 bg-white border border-border rounded p-3 hover:border-[#1e293b]/15 transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                          <span className="text-[9px] font-bold text-red-600">{item.source || "خبر"}</span>
                          <h3 className="text-[11px] font-bold leading-snug line-clamp-2 group-hover:text-red-600">{item.title}</h3>
                ...
                <div className="col-span-12 lg:col-span-6">
                  <div className="flex items-center gap-2 mb-4 border-b-2 border-[#1e293b] pb-2">
                    ...
                    <span className="ms-auto text-[10px] bg-red-600 text-white px-2 py-0.5 rounded font-bold">عاجل</span>
                  </div>
                  {breakingNews && (
                    <Link to={`/news/${breakingNews.slug || breakingNews.id}`} className="group block bg-white border border-border rounded overflow-hidden hover:border-[#1e293b]/15 transition-colors">
                      <div className="aspect-[16/10] bg-black relative overflow-hidden">
                        ...
                        <div className="absolute bottom-0 p-4 text-white">
                          <span className="bg-red-600 text-[10px] font-black px-2 py-0.5 rounded">عاجل</span>
                ...
                <div className="col-span-12 lg:col-span-3">
                  ...
                  <div className="bg-white border border-border rounded divide-y divide-black/5">
                    {trending.map((item, idx) => (
                      <Link key={item.id} to={`/news/${item.slug || item.id}`} className="group flex gap-2 p-3 hover:bg-white transition-colors">
                        <span className="size-5 grid place-items-center rounded-full bg-black text-white text-[10px] font-black shrink-0">{idx + 3}</span>
                        <div className="flex-1 min-w-0">
                          <h4 className="text-[11px] font-bold line-clamp-2 group-hover:text-red-600">{item.title}</h4>
              ...
              <div className="mt-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  {featuredPosts.map((item) => (
                    <Link key={item.id} to={`/news/${item.slug || item.id}`} className="group bg-white border border-border rounded overflow-hidden hover:border-[#1e293b]/15 transition-colors">
                      ...
                      <div className="p-3">
                        <span className="text-[9px] font-bold text-red-600">{item.source || "خبر"}</span>
                        <h3 className="text-[12px] font-bold leading-snug line-clamp-2 mt-1 group-hover:text-red-600">{item.title}</h3>
                      </div>
                    </Link>
                  ))}
                </div>
              ...
              <div className="mt-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {filteredItems.slice(4).map((item) => (
                    <Link key={item.id} to={`/news/${item.slug || item.id}`} className="group bg-white border border-border rounded p-3 hover:border-[#1e293b]/15 transition-colors">
                      <div className="flex items-center gap-2 text-[10px] text-muted-foreground mb-2">
                        <span className="bg-black text-white px-1.5 py-0.5 rounded text-[9px]">{item.source || "خبر"}</span>
                        <span>{item.published_at ? new Date(item.published_at).toLocaleDateString("ar-MA", { month: "short", day: "numeric" }) : ""}</span>
                      </div>
                      <h3 className="font-bold text-[13px] leading-snug line-clamp-2 group-hover:text-red-600">{item.title}</h3>
              ...
              <div className="mt-8 bg-black text-white rounded p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="size-10 grid place-items-center rounded bg-red-600"><Flame className="size-5" /></div>
                  <div>
            ...
            <div className="bg-white border border-dashed border-border rounded p-12 text-center">
              ...
              <button onClick={() => { setSearchQuery(""); setActiveSource("all") }} className="mt-4 rounded-full border border-border px-4 py-1.5 text-[12px] hover:border-[#1e293b]/20">إعادة ضبط</button>
            </div>
          )}
        </div>
      </main>
