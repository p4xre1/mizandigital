        ...
        <div className="container mx-auto max-w-[1280px] px-4 py-6">
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-6 border-b-2 border-[#1e293b] dark:border-white pb-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="bg-[#1e293b] dark:bg-white dark:text-black text-white text-[10px] px-2 py-0.5 rounded font-bold">{filteredItems.length} مقال</span>
              </div>
          ...
          {loading ? (
              ...
              <div className="grid grid-cols-12 gap-6">
                ...
                <div className="col-span-12 lg:col-span-3">
                  ...
                  <div className="space-y-3">
                    {editorsPicks.map((item) => (
                      <Link key={item.id} to={`/articles/${item.slug}`} className="group flex gap-3 bg-white border border-[#e7e5e4] rounded p-3 hover:border-[#1e293b]/15 transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                          <span className="text-[9px] font-bold text-[#f59e0b]">{item.category || "قانون"}</span>
                ...
                <div className="col-span-12 lg:col-span-6">
                  <div className="flex items-center gap-2 mb-4 border-b-2 border-[#1e293b] pb-2">
                    ...
                    <span className="ms-auto text-[10px] bg-[#f59e0b] text-white px-2 py-0.5 rounded font-bold">مميز</span>
                  </div>
                  {featured && (
                    <Link to={`/articles/${featured.slug}`} className="group block bg-white border border-[#e7e5e4] rounded overflow-hidden hover:border-[#1e293b]/15 transition-colors">
                      <div className="aspect-[16/10] bg-[#334155] relative overflow-hidden">
                        ...
                        <div className="absolute bottom-0 p-4 text-white">
                          <span className="bg-[#f59e0b] text-[10px] font-black px-2 py-0.5 rounded">حصري</span>
                ...
                <div className="col-span-12 lg:col-span-3">
                  ...
                  <div className="bg-white border border-[#e7e5e4] rounded divide-y divide-black/5">
                    {trending.map((item, idx) => (
                      <Link key={item.id} to={`/articles/${item.slug}`} className="group flex gap-2 p-3 hover:bg-[#fffefb] transition-colors">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  {featuredPosts.map((item) => (
                    <Link key={item.id} to={`/articles/${item.slug}`} className="group bg-white border border-[#e7e5e4] rounded overflow-hidden hover:border-[#1e293b]/15 transition-colors">
                      ...
                      <div className="p-3">
                        <span className="text-[9px] font-bold text-[#f59e0b]">{item.category}</span>
              ...
              <div className="mt-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {filteredItems.slice(4).map((item) => (
                    <Link key={item.id} to={`/articles/${item.slug}`} className="group bg-white border border-[#e7e5e4] rounded p-3 hover:border-[#1e293b]/15 transition-colors">
                      <div className="flex items-center gap-2 text-[10px] text-muted-foreground mb-2">
                        <span className="bg-[#1e293b] text-white px-1.5 py-0.5 rounded text-[9px]">{item.category || "عام"}</span>
            ...
            <div className="bg-white border border-dashed border-[#e7e5e4] rounded p-12 text-center">
              ...
              <button onClick={() => { setSearchQuery(""); setActiveCategory("all") }} className="mt-4 rounded-full border border-[#e7e5e4] px-4 py-1.5 text-[12px] hover:border-[#1e293b]/20">إعادة ضبط</button>
            </div>
          )}
        </div>
      </main>
