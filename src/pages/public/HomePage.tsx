...
export function HomePage() {
  ...
  const homepageSchemas = [
    { "@context": "https://schema.org", "@type": "WebSite", "name": "منصة الميزان الرقمية", "alternateName": "Mizan Digital Platform", "url": "https://www.mizan.page", "description": "المنصة الرقمية الأولى للعلوم القانونية والتشريع المغربي، توفر الأرشيف الدراسي والمعجم القانوني ودليل الكليات.", "inLanguage": "ar-MA", "potentialAction": { "@type": "SearchAction", "target": "https://www.mizan.page/lexicon?q={search_term_string}", "query-input": "required name=search_term_string" } },
    { "@context": "https://schema.org", "@type": "Organization", "name": "منصة الميزان الرقمية", "url": "https://www.mizan.page", "logo": "https://www.mizan.page/og-default.jpg", "sameAs": [] }
  ]

  return (
    <>
      <SEOHead title="الرئيسية - المرفق القانوني الأول بالمغرب" description="منصة الميزان الرقمية توفر الأرشيف الدراسي، والمدونة الشاملة، والمعجم القانوني المغربي الموحد، ودليل كليات الحقوق للطلبة والمهنيين." keywords={["القانون المغربي", "منصة الميزان الرقمية", "الأرشيف القانوني المغربي", "مدونة الشغل المغربية", "المعجم القانوني المغربي", "كليات الحقوق بالمغرب"]} schema={homepageSchemas} />
      <main className="min-h-screen bg-background text-foreground" dir="rtl">
        <section className="relative overflow-hidden border-b border-border bg-gradient-to-b from-primary/5 via-background to-background py-16 md:py-24"><div className="container mx-auto max-w-6xl px-4"><div className="mx-auto max-w-3xl text-center"><h1 className="text-3xl font-black tracking-tight text-foreground sm:text-5xl md:text-6xl leading-tight">مرجعك الأول في <span className="text-primary">القانون المغربي</span> والأرشيف الأكاديمي</h1><p className="mt-6 text-base text-muted-foreground sm:text-lg leading-relaxed max-w-2xl mx-auto">نوفر للطلبة والباحثين والمهنيين أرشيفاً دراسياً كاملاً، معجماً قانونياً شاملاً، ومتابعة حية للمستجدات التشريعية.</p><div className="mt-8 flex flex-wrap items-center justify-center gap-4"><Link to="/lexicon" className="inline-flex items-center gap-2 rounded-xl bg-primary px-6 py-3.5 text-sm font-bold text-primary-foreground shadow-md transition hover:opacity-90 hover:shadow-lg"><BookOpen size={18}/><span>تصفح المعجم القانوني</span></Link><Link to="/schools" className="inline-flex items-center gap-2 rounded-xl border border-border bg-card px-6 py-3.5 text-sm font-bold text-foreground transition hover:bg-muted"><GraduationCap size={18}/><span>دليل كليات الحقوق</span></Link></div></div></div></section>
        ...
        <section className="py-16 border-t border-border"><div className="container mx-auto max-w-6xl px-4"><div className="mb-10 text-center"><div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-3.5 py-1 text-xs font-semibold text-primary border border-primary/20 mb-3"><Sparkles size={14}/><span>محدَّث تلقائياً</span></div><h2 className="text-2xl font-black text-foreground sm:text-3xl">أحدث المستجدات</h2><p className="mt-2 text-sm text-muted-foreground">آخر ما نُشر بمختلف أقسام المنصة، مرتّب حسب الحداثة وتنوّع المواضيع</p></div>
          {feedLoading ? <div className="flex h-40 items-center justify-center"><Loader2 className="size-8 animate-spin text-primary"/></div> : <div className="space-y-12">
            {latestArticles.length > 0 && <div><div className="mb-4 flex items-center justify-between"><h3 className="flex items-center gap-2 text-base font-extrabold"><FileText size={18} className="text-primary"/>أحدث المقالات</h3><Link to="/articles" className="text-xs font-bold text-primary">عرض الكل <ArrowLeft size={13}/></Link></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">{latestArticles.map(item=><Link key={item.slug} to={`/articles/${item.slug}`} className="group flex flex-col gap-2 rounded-xl border border-border bg-card p-4 transition hover:border-primary/50 hover:shadow-sm">{item.category&&<span className="w-fit rounded bg-primary/10 px-2 py-0.5 text-[10px] font-semibold text-primary">{item.category}</span>}<h4 className="text-sm font-bold line-clamp-2">{item.title}</h4>{item.summary&&<p className="line-clamp-2 text-[11px] text-muted-foreground">{item.summary}</p>}</Link>)}</div></div>}
            {latestNews.length > 0 && <div><div className="mb-4 flex items-center justify-between"><h3 className="flex items-center gap-2 text-base font-extrabold"><Newspaper size={18} className="text-primary"/>أحدث الأخبار</h3><Link to="/news" className="text-xs font-bold text-primary">عرض الكل <ArrowLeft size={13}/></Link></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">{latestNews.map(item=><Link key={item.slug} to={`/news/${item.slug}`} className="group flex flex-col gap-2 rounded-xl border border-border bg-card p-4 transition hover:border-primary/50 hover:shadow-sm">{item.category&&<span className="w-fit rounded bg-primary/10 px-2 py-0.5 text-[10px] font-semibold text-primary">{item.category}</span>}<h4 className="text-sm font-bold line-clamp-2">{item.title}</h4>{item.summary&&<p className="line-clamp-2 text-[11px] text-muted-foreground">{item.summary}</p>}</Link>)}</div></div>}
            {latestEvents.length > 0 && <div><div className="mb-4 flex items-center justify-between"><h3 className="flex items-center gap-2 text-base font-extrabold"><Calendar size={18} className="text-primary"/>أحدث الندوات والفعاليات</h3><Link to="/events" className="text-xs font-bold text-primary">عرض الكل <ArrowLeft size={13}/></Link></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-3">{latestEvents.map(item=><Link key={item.slug} to={`/events/${item.slug}`} className="group flex flex-col gap-2 rounded-xl border border-border bg-card p-4"><span className="w-fit inline-flex items-center gap-1 rounded bg-primary/10 px-2 py-0.5 text-[10px] font-semibold text-primary"><Video size={10}/>{item.category||"فعالية"}</span><h4 className="text-sm font-bold line-clamp-2">{item.title}</h4></Link>)}</div></div>}
            {latestDocs.length > 0 && <div><div className="mb-4 flex items-center justify-between"><h3 className="flex items-center gap-2 text-base font-extrabold"><Download size={18} className="text-primary"/>أحدث وثائق الأرشيف</h3><Link to="/archive" className="text-xs font-bold text-primary">عرض الكل <ArrowLeft size={13}/></Link></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-3">{latestDocs.map(item=><Link key={item.slug} to={`/download/${item.id}`} className="group flex flex-col gap-2 rounded-xl border border-border bg-card p-4"><span className="w-fit inline-flex items-center gap-1 rounded bg-primary/10 px-2 py-0.5 text-[10px] font-semibold text-primary"><FileText size={10}/>{item.category?`الفصل ${item.category}`:"PDF"}</span><h4 className="text-sm font-bold line-clamp-2">{item.title}</h4></Link>)}</div></div>}
            {latestTerms.length > 0 && <div><div className="mb-4 flex items-center justify-between"><h3 className="flex items-center gap-2 text-base font-extrabold"><Scale size={18} className="text-primary"/>أحدث المصطلحات القانونية</h3><Link to="/lexicon" className="text-xs font-bold text-primary">عرض الكل <ArrowLeft size={13}/></Link></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-3">{latestTerms.map(item=><Link key={item.slug} to={`/lexicon/${item.slug}`} className="group flex flex-col gap-2 rounded-xl border border-border bg-card p-4">{item.category&&<span className="w-fit rounded bg-primary/10 px-2 py-0.5 text-[10px] font-semibold text-primary">{item.category}</span>}<h4 className="text-sm font-bold">{item.title}</h4>{item.summary&&<p className="text-[11px] text-muted-foreground" dir="ltr">{item.summary}</p>}</Link>)}</div></div>}
          </div>}
        </div></section>
        ...
        <section className="py-12 bg-primary text-primary-foreground"><div className="container mx-auto max-w-6xl px-4"><div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center"><div><div className="text-3xl font-black sm:text-4xl">+10,000</div><div className="mt-1 text-xs opacity-90 font-medium">مستفيد ومتابع</div></div><div><div className="text-3xl font-black sm:text-4xl">+1,500</div><div className="mt-1 text-xs opacity-90 font-medium">مصطلح قانوني موحد</div></div><div><div className="text-3xl font-black sm:text-4xl">+25</div><div className="mt-1 text-xs opacity-90 font-medium">كلية ومؤسسة جامعية</div></div><div><div className="text-3xl font-black sm:text-4xl">100%</div><div className="mt-1 text-xs opacity-90 font-medium">محتوى مفتوح ومجانى</div></div></div></div></section>
      </main>
    </>
  )
}
