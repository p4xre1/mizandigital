...
import {
  ...
  Columns2, Sun, Moon, MonitorSmartphone, X, Twitter, Facebook, Linkedin, Share2, ThumbsUp,
} from "lucide-react"
...
export function ArticlePage({ slug: propSlug }: ArticlePageProps) {
  ...
  const tocPanel = (
    <nav className="space-y-1 text-[13px] max-h-[60vh] overflow-y-auto pl-1">
      <a href="#top" className={`flex items-center gap-2.5 rounded-xl py-2 px-3 font-bold transition-colors ${activeSection === "top" ? "bg-primary/10 text-primary border border-primary/20 shadow-sm" : "text-foreground hover:bg-muted"}`}><span className={`size-2 rounded-full ${activeSection === "top" ? "bg-primary" : "bg-border"}`} />بداية المقال</a>
      {sectionsList.map((section, idx) => {
        const isActive = activeSection === section.id
        return <a key={idx} href={`#${section.id}`} className={`flex items-center gap-2.5 rounded-xl py-2 px-3 transition-colors ${section.level === 3 ? "mr-4 text-[12px]" : ""} ${isActive ? "bg-primary/10 text-primary font-bold border border-primary/20 shadow-sm" : "text-muted-foreground hover:text-foreground hover:bg-muted"}`}><span className={`size-1.5 rounded-full ${isActive ? "bg-primary" : "bg-border"}`} /><span className="truncate">{section.title}</span></a>
      })}
    </nav>
  )
  ...
  return (
    <>
      <AEOHead
        ...
        schema={detailSchema}
      />
      <div className="fixed inset-x-0 top-0 z-50 h-[3px] bg-transparent"><div className="h-full bg-primary transition-[width] duration-150 ease-out shadow-sm" style={{ width: `${readingProgress}%` }} /></div>
      <div id="top" />
      <div className="min-h-screen bg-background">
      <main className={`mx-auto ${maxWidthClass} px-3 md:px-6 py-6 md:py-10 transition-colors duration-200 ease-out`} dir="rtl">
        <div className="mb-6 flex items-center justify-between gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap"><ArticleTranslateWidget /><div className="flex items-center gap-2 xl:hidden"><button onClick={() => { setShowContents((v) => !v); setShowAppearance(false) }} className={`inline-flex items-center gap-1.5 rounded-xl border px-3.5 py-2 text-[11px] font-bold  ${showContents ? "border-primary bg-primary text-primary-foreground shadow" : "border-border bg-card text-muted-foreground hover:text-foreground"}`}><List size={14} />المحتويات<ChevronDown size={12} className={`transition-transform ${showContents ? "rotate-180" : ""}`} /></button><button onClick={() => { setShowAppearance((v) => !v); setShowContents(false) }} className={`inline-flex items-center gap-1.5 rounded-xl border px-3.5 py-2 text-[11px] font-bold  ${showAppearance ? "border-primary bg-primary text-primary-foreground shadow" : "border-border bg-card text-muted-foreground hover:text-foreground"}`}><SlidersHorizontal size={14} />المظهر</button></div></div>
        </div>
        {(showContents || showAppearance) && <div className="mb-6 xl:hidden animate-[fadeUp_0.4s_cubic-bezier(0.16,1,0.3,1)]"><div className="rounded-2xl border border-border/50 bg-card p-5 shadow-sm relative overflow-hidden"><div className="absolute inset-x-0 top-0 h-[1px] bg-border opacity-60" /><button onClick={() => { setShowContents(false); setShowAppearance(false) }} className="absolute left-4 top-4 grid size-8 place-items-center rounded-full bg-muted text-muted-foreground hover:bg-foreground hover:text-background"><X size={14} /></button>{showContents && <><h3 className="font-black text-sm mb-4 flex items-center gap-2"><div className="grid size-7 place-items-center rounded-lg bg-primary/10 text-primary"><List size={14} /></div>محتويات المقال</h3>{sectionsList.length > 0 ? tocPanel : <p className="text-[12px] text-muted-foreground">لا توجد عناوين فرعية.</p>}</>}{showAppearance && <><h3 className="font-black text-sm mb-4 flex items-center gap-2"><div className="grid size-7 place-items-center rounded-lg bg-primary/10 text-primary"><SlidersHorizontal size={14} /></div>خيارات القراءة</h3>{appearancePanel}</>}</div></div>}
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 md:gap-8 items-start">
          <div className="hidden xl:block xl:col-span-3 xl:order-1"><div className="sticky top-24 space-y-4">{sectionsList.length > 0 && <div className="group rounded-2xl border border-border/50 bg-card p-5 shadow-sm transition-colors"><div className="absolute inset-x-0 top-0 h-[1px] bg-primary/30 opacity-0 group-hover:opacity-100 transition-opacity" /><h3 className="flex items-center gap-2 pb-3 mb-3 border-b border-border/50 font-black text-[13px]"><div className="grid size-6 place-items-center rounded-lg bg-primary/10 text-primary"><List size={14} /></div>محتويات المقال<span className="mr-auto rounded-full bg-primary/10 px-2 py-0.5 text-[10px] text-primary">{sectionsList.length}</span></h3>{tocPanel}</div>}<div className="rounded-2xl border border-border/50 bg-primary/[0.04] p-4 "><h4 className="text-[11px] font-black mb-3">إحصائيات القراءة</h4><div className="space-y-2.5"><div className="flex justify-between text-[11px]"><span className="text-muted-foreground">التقدم</span><span className="font-bold text-primary">{Math.round(readingProgress)}%</span></div><div className="h-1.5 rounded-full bg-muted overflow-hidden"><div className="h-full bg-primary transition-colors" style={{ width: `${readingProgress}%` }} /></div><div className="flex justify-between text-[11px]"><span className="text-muted-foreground">وقت القراءة</span><span className="font-bold">{article.readingTime || "5 د"}</span></div></div></div></div></div>
          <article ref={articleBodyRef} className="xl:col-span-6 order-1 xl:order-2 group relative overflow-hidden rounded-2xl border border-border/50 bg-card shadow-sm transition-colors duration-200">
            <div className="absolute inset-x-0 top-0 h-[2px] bg-primary opacity-80" />
            <div className="p-6 md:p-10">
            <header className="space-y-5 mb-10">
              <div className="flex items-center gap-2 flex-wrap">{article.category && <span className="inline-flex items-center gap-1.5 rounded-full bg-primary/10 border border-primary/20 px-3.5 py-1.5 text-[11px] font-black text-primary"><div className="size-1.5 rounded-full bg-primary" />{article.category}</span>}{formattedDate && <span className="inline-flex items-center gap-1.5 rounded-full bg-muted border border-border/50 px-3 py-1 text-[11px] font-bold text-muted-foreground"><Calendar size={12} />{formattedDate}</span>}{article.readingTime && <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-500/10 border border-amber-500/20 px-3 py-1 text-[11px] font-bold text-amber-700"><BookOpen size={12} />{article.readingTime}</span>}{article.sourceTable && <ViewCounter table={article.sourceTable} slug={article.slug} />}</div>
              <h1 className="text-[1.7rem] md:text-[2.2rem] font-black leading-[1.15] tracking-tight">{article.title}</h1>
              {article.summary && <div className="relative rounded-2xl border border-primary/10 bg-primary/[0.04] p-4 md:p-5"><p className="text-[13px] md:text-[14px] leading-7 text-foreground/80 font-medium">{article.summary}</p></div>}
              {article.keywords && article.keywords.length > 0 && <div className="flex flex-wrap items-center gap-2"><span className="text-[11px] font-black text-muted-foreground flex items-center gap-1.5"><div className="grid size-5 place-items-center rounded-full bg-primary/10 text-primary"><Tag size={10} /></div>الكلمات المفتاحية:</span>{article.keywords.map((kw, idx) => <span key={idx} className="rounded-full bg-muted hover:bg-primary/10 hover:text-primary border border-border hover:border-primary/20 px-3 py-1 text-[11px] font-bold text-muted-foreground transition-colors">#{kw}</span>)}</div>}
            </header>
            {article.image && <div className="mb-10 group/img relative overflow-hidden rounded-2xl border border-border/50 bg-muted shadow-sm"><img src={article.image} alt={article.imageAlt || article.title} className="max-h-[480px] w-full object-cover transition-transform duration-[1.5s] ease-out group-hover/img:scale-[1.02]" /></div>}
            {article.highlights && article.highlights.length > 0 && <div className="mb-10 relative overflow-hidden rounded-2xl border border-primary/15 bg-primary/[0.04] p-6"><h3 className="flex items-center gap-2 text-[13px] font-black text-primary mb-4"><div className="grid size-6 place-items-center rounded-lg bg-primary/10"><BookOpen className="size-3.5" /></div>أبرز النقاط</h3><ul className="space-y-3">{article.highlights.map((h, idx) => <li key={idx} className="flex items-start gap-3 text-[13px] leading-6 text-foreground/90"><span className="mt-2 size-1.5 rounded-full bg-primary shrink-0" /><span className="flex-1">{h}</span></li>)}</ul></div>}
            ...
            <div className="mt-10 rounded-2xl border border-border/50 bg-muted/30 p-6"><div className="flex items-center justify-between gap-3 mb-4"><h3 className="text-[14px] font-black flex items-center gap-2"><div className="grid size-7 place-items-center rounded-full bg-amber-500/10 text-amber-600"><ThumbsUp className="size-3.5" aria-hidden="true" /></div>هل كان هذا المقال مفيداً؟</h3><ReportDialog targetType={article.sourceTable === "news" ? "news" : "article"} targetId={article.slug} /></div><ReactionBar targetType={article.sourceTable === "news" ? "news" : "article"} targetId={article.slug} /></div>
            {article.sourceTable && <div className="mt-8"><CommentSection table={article.sourceTable} slug={article.slug} /></div>}
            </div>
          </article>
          <div className="hidden xl:block xl:col-span-3 order-3"><div className="sticky top-24 space-y-4"><div className="rounded-2xl border border-border/50 bg-card p-5 shadow-sm"><h3 className="flex items-center gap-2 pb-3 mb-4 border-b border-border/50 font-black text-[13px]"><div className="grid size-6 place-items-center rounded-lg bg-primary/10 text-primary"><SlidersHorizontal size={14} /></div>خيارات القراءة</h3>{appearancePanel}</div><div className="rounded-2xl border border-border bg-muted p-5"><h4 className="text-[12px] font-black mb-3">شارك المقال</h4><div className="grid grid-cols-4 gap-2">{[{ Icon: Twitter, label: "مشاركة على إكس" }, { Icon: Facebook, label: "مشاركة على فيسبوك" }, { Icon: Linkedin, label: "مشاركة على لينكد إن" }, { Icon: Share2, label: "نسخ رابط المقال" }].map(({ Icon, label }, i) => <button key={i} type="button" aria-label={label} title={label} className="grid size-10 place-items-center rounded-xl bg-card border border-border hover:bg-foreground hover:text-background transition-colors"><Icon className="size-4" aria-hidden="true" /></button>)}</div></div></div></div>
        </div>
        {relatedArticles.length > 0 && <section className="mt-20"><div className="flex items-center justify-between mb-8"><div className="flex items-center gap-3"><div className="grid size-10 place-items-center rounded-xl bg-primary text-white shadow-sm"><BookOpen className="size-5" /></div><div><h3 className="text-lg font-black">مقالات ذات صلة</h3><p className="text-[12px] text-muted-foreground">مختارة حسب اهتماماتك</p></div></div><Link to="/articles" className="group inline-flex items-center gap-2 rounded-full border border-border bg-card px-4 py-2 text-xs font-bold hover:border-primary/20"><span>عرض الكل</span><ArrowLeft size={14} className="group-hover:-translate-x-1 transition-transform" /></Link></div><div className="grid grid-cols-1 md:grid-cols-3 gap-6">{relatedArticles.map((item) => <Link key={item.id} to={`/articles/${item.slug}`} className="group relative overflow-hidden rounded-2xl border border-border/50 bg-card p-5 shadow-sm hover:border-primary/20 transition-colors duration-200"><div className="absolute inset-x-0 top-0 h-[2px] bg-primary opacity-0 group-hover:opacity-100 transition-opacity" /><div className="space-y-3">{item.category && <span className="inline-flex rounded-full bg-primary/10 border border-primary/20 px-2.5 py-1 text-[10px] font-black text-primary">{item.category}</span>}<h4 className="font-black text-[14px] leading-snug line-clamp-2 group-hover:text-primary transition-colors">{item.title}</h4>{item.summary && <p className="text-[12px] text-muted-foreground line-clamp-2 leading-6">{item.summary}</p>}</div><div className="mt-4 flex items-center justify-between pt-3 border-t border-border/50 text-[11px] font-bold text-primary"><span>قراءة المقال</span><div className="grid size-7 place-items-center rounded-full bg-primary/10 group-hover:bg-primary group-hover:text-primary-foreground transition-colors"><ArrowLeft size={14} /></div></div></Link>)}</div></section>}
      </main></div>
    </>
  )
}
