...
export default function Profile() {
  ...
  useEffect(() => {
    async function loadUserProfileAndData() {
      ...
      try {
        ...
        const { data: savedItems } = await (supabase.from("user_saved_items") as any)
            .select("item_id, item_type, created_at, title")
            .eq("user_id", user.id);

        if (savedItems && (savedItems as DbSavedItem[]).length > 0) {
          const items = savedItems as DbSavedItem[];
          const articlesList: Article[] = items
        ...
        const { data: userResumes } = await (supabase.from("user_resumes") as any)
            .select("id, file_name, file_size")
            .eq("user_id", user.id);

        if (userResumes) {
          const resumesList = userResumes as DbUserResume[];
          setResumes(
              resumesList.map((r) => ({
                id: r.id,
                name: r.file_name,
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (isSupabaseConfigured && userId) {
      try {
        ...
        if (uploadError) {
          console.warn("Storage upload failed, attempting Base64 fallback...", uploadError);
  ...
  // Freeze Account (Explicit `window.` calls prevent UMD warnings)
