...
const I18N_DICT = {
  ar: {
    ...
    unban: "إلغاء الحظر",
    delete: "حذف الحساب",
    confirmDelete: "هل أنت ألكيد من رغبتك في حذف هذا المستخدم نهائياً؟",
...
export default function AdminUsers() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      if (!error && data) {
        ...
        if (isMounted.current) setDbUsers(formatted);
      }
    } catch (err) {
  ...
  const activeUsersList: FormattedAdminUser[] = dbUsers.length > 0
      ? dbUsers
      : cms.users.map((u) => ({
        id: u.id,
        name: u.name,
