...
export default function ArticlesList() {
  ...
  const filteredArticles = useMemo(() => {
    return articles.filter((article) => {
      ...
      const matchesSearch =
        ...
        artCat.includes(searchQuery.toLowerCase()) ||
  ...
  return (
    ...
    >
      ...
      {/* Category Tabs (Syncs with Navbar Routes) */}
        ...
        {CATEGORIES.map((cat) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl text-xs sm:text-sm font-semibold transition-all whitespace-nowrap shadow-xs cursor-pointer ${
                isActive
      ...
      <div className="max-w-md mx-auto relative">
        <Search
          size={18}
          className="absolute top-1/2 -translate-y-1/2 ltr:left-4 rtl:right-4 text-muted-foreground pointer-events-none"
        />
        <input
          type="text"
          value={searchQuery}
          ...
          className="w-full text-sm ltr:pl-11 rtl:pr-11 ltr:pr-4 rtl:pl-4 py-3 rounded-2xl border border-border bg-card text-foreground shadow-xs outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
        />
      </div>
