...
const ChartContainer = React.forwardRef<ChartContainerElement, ChartContainerProps>(
  ({ id, className, children, config, ...props }: ChartContainerProps, ref: React.ForwardedRef<ChartContainerElement>): React.JSX.Element => {
...
export type ChartTooltipContentElement = HTMLDivElement;
...
>(
  (
    ...
    ref: React.ForwardedRef<ChartTooltipContentElement>
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl ltr:text-left rtl:text-right",
          className
        )}
        {...props}
      >
...
export type ChartLegendContentElement = HTMLDivElement;
...
>(
  (
    ...
    ref: React.ForwardedRef<ChartLegendContentElement>
  ): React.JSX.Element | null => {
    const { config } = useChart();
