...
const CATEGORIES: Category[] = [
  { slug: "family-law", ar: "قانون الأسرة", fr: "Droit de la famille", en: "Family Law", es: "Derecho de familia" },
  { slug: "criminal-law", ar: "القانون الجنائي", fr: "Droit pénal", en: "Criminal Law", es: "Derecho penal" },
  { slug: "civil-law", ar: "القانون المدني", fr: "Droit civil", en: "Civil Law", es: "Derecho civil" },
  { slug: "commercial-law", ar: "القانون التجاري", fr: "Droit commercial", en: "Commercial Law", es: "Derecho comercial" },
  { slug: "administrative-law", ar: "القانون الإداري", fr: "Droit administratif", en: "Administrative Law", es: "Derecho administratif" },
  { slug: "constitutional-law", ar: "القانون الدستوري", fr: "Droit constitutionnel", en: "Constitutional Law", es: "Derecho constitucional" },
];
      ...
      <div className="px-4 py-2.5 max-w-7xl mx-auto flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-1 text-sm font-semibold">
          ...
          >
            ...
            {desktopLibraryOpen && (
              <div className="absolute top-full rtl:right-0 ltr:left-0 w-[540px] bg-card border border-border rounded-xl shadow-xl p-5 grid grid-cols-2 gap-6 z-50 animate-in fade-in-50 zoom-in-95">
                <div>
                  <div className="flex items-center gap-1.5 text-xs font-bold text-primary uppercase mb-3 pb-1 border-b border-border">
                    <Scale size={14} />
                    <span>{lang === "ar" ? "التخصصات القانونية" : "Disciplines"}</span>
                  </div>
                  <div className="space-y-1">
                    {CATEGORIES.map((cat) => (
                      <Link
                        key={cat.slug}
                        to={`/${lang}/library/${cat.slug}`}
                ...
                <div className="bg-muted/50 p-2 space-y-1 border-t border-border">
                  {CATEGORIES.map((cat) => (
                    <Link
                      ...
                      to={`/${lang}/library/${cat.slug}`}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="flex items-center gap-2 min-h-[44px] px-3 rounded-lg text-xs font-medium hover:bg-card active:bg-card"
                    >
                      <FileText size={14} className="text-muted-foreground shrink-0" />
                      <span>{cat[lang]}</span>
                    </Link>
