...
// Google AdSense Client ID from Environment or Fallback
const ADSENSE_CLIENT_ID = import.meta.env.VITE_ADSENSE_CLIENT_ID || "ca-pub-0000000000000000";

/**
 * High-Speed & Safe AdSense Native Banner Unit inside Hero
 */
const HeroAdBanner: React.FC<{ slotId?: string }> = ({ slotId = "1020304050" }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto my-3 overflow-hidden rounded-2xl bg-slate-800/40 border border-slate-700/50 backdrop-blur-md p-2 text-center">
      <span className="text-[9px] font-bold tracking-widest text-slate-400 uppercase mb-1 block opacity-70">
...
export const HeroSection: React.FC<HeroSectionProps> = ({ lang, dir }) => {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      navigate(`${getPath("/archive")}?q=${encodeURIComponent(searchQuery.trim())}`);
    }
  };
  ...
  return (
    <section
      aria-label={t.title}
      className="relative w-full min-h-[520px] sm:min-h-[620px] flex flex-col justify-center overflow-hidden border-b border-slate-200 dark:border-slate-800 bg-slate-950 text-white select-none transition-colors"
      dir={dir}
    >
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-14 text-center space-y-5 sm:space-y-6">
        ...
        >
          <div className="relative flex items-center bg-slate-900/90 border border-slate-700/80 rounded-2xl p-1.5 shadow-xl focus-within:border-blue-500 transition-all backdrop-blur-xl">
            ...
            <button
              type="submit"
              className="min-h-[44px] px-4 sm:px-6 bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs sm:text-sm rounded-xl transition active:scale-95 shrink-0 flex items-center justify-center gap-1.5 shadow-sm cursor-pointer"
            >
        ...
        {/* Integrated AdSense Hero Native Slot */}
      ...
      <div className="relative z-10 w-full border-t border-slate-800/80 bg-slate-900/60 backdrop-blur-md py-4 mt-4">
        <div className="max-w-5xl mx-auto px-4 grid grid-cols-3 gap-2 sm:gap-6 text-center">
          {t.stats.map((stat, i) => {
            ...
            return (
              <div key={i} className="flex flex-col items-center justify-center space-y-0.5">
