...
export function RichTextEditor({ value, onChange, dir }: Props) {
    ...
    // Custom Inline Prompt State for Mobile
    const [promptType, setPromptType] = useState<PromptType>(null);
    const [promptUrl, setPromptUrl] = useState("");
    const [securityWarning, setSecurityWarning] = useState<string | null>(null);

    // AdSense Client ID & Slot ID resolution
    const adsenseClientId =
        import.meta.env.VITE_GOOGLE_ADSENSE_CLIENT_ID || "ca-pub-1749032173858747";
    const adsenseSlotId = import.meta.env.VITE_GOOGLE_ADSENSE_SLOT_ID || "";

    // Check if a valid, real slot ID is available before initializing
    const isValidAdConfig =
        Boolean(adsenseClientId) &&
        !adsenseClientId.includes("xxxxxxxx") &&
        Boolean(adsenseSlotId) &&
        adsenseSlotId.length > 0;

    // Safe Google AdSense Loader
    useEffect(() => {
        if (!isValidAdConfig) return;
        ...
        if (promptType === "link") {
            ...
            if (embed && isSafeHttpUrl(embed)) {
                ...
                (document as any).execCommand(
                    "insertHTML",
                    false,
                    `<figure class="my-3"><iframe src="${embed}" width="100%" height="280" allowfullscreen title="embedded-video" class="rounded-2xl border border-slate-200 dark:border-slate-800"></iframe></figure><p><br/></p>`
                );
                emit();
            } else {
    ...
    return (
        <div className="border border-border rounded-3xl overflow-hidden bg-card focus-within:ring-2 focus-within:ring-primary focus-within:border-transparent transition-all shadow-xs font-sans">
            ...
            {/* Google AdSense Academic Tools Banner (Only renders if a valid slot exists) */}
            {isValidAdConfig && (
