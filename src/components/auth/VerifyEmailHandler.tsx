  ...
  useEffect(() => {
    const handleEmailVerification = async () => {
      ...
      const failVerification = (msg?: string) => {
        ...
        setErrorMessage(msg || fallbackMsg);
        setStatus("error");
      ...
      try {
        ...
        if (error) {
          failVerification(errorDescription || "Verification link is invalid or expired.");
          return;
        }
        ...
        if (code) {
          ...
          if (sessionError) {
            failVerification(sessionError.message);
            return;
          }
        }
        ...
        if (userError || !user) {
          failVerification("Unable to verify user session.");
          return;
        }

        if (!user.email_confirmed_at) {
          failVerification("Email verification failed. Please try again.");
          return;
        }
