...
interface ImageUploadFieldProps {
  ...
  /**
   * نص بديل (Alt Text) لهذه الصورة — اختياري. عند تمرير altValue/onAltChange
   * يظهر حقل مخصص لإدارة الـ alt وحفظه بشكل منفصل عن رابط الصورة، لضمان
   * فهرسة سيو صحيحة للصور (Google Images) وتوافق مع معايير إمكانية الوصول.
   */
  altValue?: string
  onAltChange?: (alt: string) => void
  /** نص يظهر كمثال داخل حقل الـ alt عند تركه فارغاً (عادة عنوان المقال/الخبر) */
...
export function ImageUploadField({
  ...
  return (
    <div className="space-y-1.5">
      ...
      {onAltChange && (
        <div className="space-y-1 rounded-lg border border-dashed border-border bg-muted/20 p-2.5">
          <label className="flex items-center gap-1.5 text-[11px] font-semibold text-muted-foreground">
            النص البديل للصورة (Alt Text)
            <span className="font-normal text-muted-foreground/70">— لتحسين ظهورها في نتائج بحث الصور</span>
          </label>
          <input
            type="text"
            value={altValue || ""}
            onChange={(e) => onAltChange(e.target.value)}
            placeholder={altPlaceholder || "وصف مختصر ودقيق لمحتوى الصورة..."}
          ...
          {!altValue && (
            <p className="text-[10px] text-muted-foreground/80">
              إن تُرك فارغاً، سيُستخدم العنوان الرئيسي تلقائياً كنص بديل.
            </p>
          )}
        </div>
      )}
      ...
      {value && (
        <div className="relative mt-1 inline-block">
          <img src={value} alt={altValue || ""} className="h-20 w-32 rounded-lg border border-border object-cover" />
          <button
            type="button"
            onClick={() => onChange("")}
