...
3. **`script-src 'unsafe-inline'` في CSP**: ما زال موجوداً في `public/_headers`
   لأن سكربت إقلاع Google Analytics في `index.html` (تعريف `gtag()` وقراءة
   قرار الموافقة من localStorage) يُحقن inline. إزالته يتطلب nonces.
   (كانت وحدات Adsterra سبباً ثانياً؛ أُزيلت الإعلانات بالكامل فلم يبقَ سببها.)
