...
3. **`script-src 'unsafe-inline'` في CSP**: ما زال موجوداً في `public/_headers`
   لأن سكربتات GA/Adsterra تُحقن inline. إزالته يتطلب nonces.
4. **إطار الإعلانات `/ads/frame` له CSP مرنة عمداً** (`script-src *`) وهو
   معزول في iframe — انظر التعليقات في `public/_headers`.
5. **فلاتر السبام قائمة على الأنماط**: مهاجم مصمم يتجاوزها. الحماية الفعلية
